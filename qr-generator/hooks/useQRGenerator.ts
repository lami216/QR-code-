...
import { createInitialQRContent, type QRType } from "../lib/qr/modes";
...
export const useQRGenerator = (initialType?: QRType) => {
  ...
  const generateQR = useCallback(
    (newContent?: QRContent, newStyling?: QRStyling) => {
      ...
      debounceRef.current = setTimeout(() => {
        try {
          const qrDataURL = QRGenerator.generateQRCode(
            currentContent,
            currentStyling,
          );
          setQrCode(qrDataURL);
  ...
  const generateQRImmediate = useCallback(
    (newContent?: QRContent, newStyling?: QRStyling) => {
      if (debounceRef.current) {
        clearTimeout(debounceRef.current);
      }
  ...
  const resetGenerator = useCallback(() => {
    setContent(createInitialQRContent(initialType));
    setStyling({
      foreground: "#000000",
      background: "#ffffff",
      transparent: false,
